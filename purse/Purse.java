...
public class Purse extends Applet {
	...
	public void process(APDU apdu) {
		...
		if(!rc){
			//返回数据
			for(byte i = 0 ; i < papdu.le; i++){ //以响应数据长度为主
	...
	private boolean create_file() {
		switch(papdu.pdata[0]){ //data第一位表示创建的文件类型            
	...
	private boolean init_purchase(){
		...
		num = keyfile.findkey(papdu.pdata[0]);//根据tag寻找密钥并返回记录号
