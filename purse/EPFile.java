 ...
 */
	...
	public final short load(byte[] data){
		...
		Util.arrayCopyNonAtomic(pTemp16, (short)5, pTemp82, (short)0, (short)8);//去前五位密钥头
	...
	public final short decrease(byte[] data, boolean flag){
		...
		for(i = 3; i >= 0; i --){
			t1 = (short)(EP_balance[(short)i] & 0xFF);
			t2 = (short)(data[i] & 0xFF);//data[i]为一个byte，short两个byte，补0xFF
			if(flag){
				if(t1 > t2){
	...
	public final short init4purchase(short num, byte[] data){
		...
		//响应数据
		Util.arrayCopyNonAtomic(EP_balance, (short)0, data, (short)0, (short)4);      //电子钱包余额
		Util.arrayCopyNonAtomic(EP_offline, (short)0, data,  (short)4, (short)2);      //电子钱包联机交易序号
	...
	public final short purchase(byte[] data){
		...
		//EnCipher.gmac4(pTemp82, pTemp32, (short)0x0F, pTemp41);
		EnCipher.gmac4(pTemp82, pTemp32, (short)18, pTemp41);//生成MAC1，储存在pTemp41，data为pTemp32
		...
		Util.arrayCopyNonAtomic(data, (short)4, pTemp32, (short)15, (short)7);         //终端交易日期与时间
		...
		Util.arrayCopyNonAtomic(pTemp16, (short)13, temp, (short)0, (short)8);//pTemp16[13]开始才是密钥
		...
		*/
		...
		EnCipher.gmac4(pTemp82, pTemp32, (short)0x16, data);//获得tac， 复制给data
